...
const PAGE_CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Hardware table ───────────────────────────────────────────────────────── */
  .hw-table { width: 100%; border-collapse: collapse; margin: 8px 0; }
  .hw-table th {
  ...
  .hw-table tr:nth-child(even) td { background: #FFF8F0; }
  .hw-cat-header td {
    background: #FFF3E0; font-weight: bold; color: #E65100;
    font-size: 9pt; padding: 6px 8px;
    border-bottom: 2px solid #FFCC80;
  }
  .hw-note { font-size: 7.5pt; color: #9E9E9E; font-style: italic; }
...
export interface PdfExportOptions {
  ...
  settings: OptimizationSettings;
  /** Optional hardware shopping list. If provided, renders a hardware section in the PDF. */
  hardware?: HardwareRecommendation;
}

function renderHardwareSection(hardware: HardwareRecommendation): string {
  if (hardware.items.length === 0) return '';

  const categories: HardwareItem['category'][] = ['fasteners', 'hinges', 'slides', 'accessories'];
  const rows: string[] = [];
  ...
  for (const cat of categories) {
    ...
    rows.push(
      `<tr class="hw-cat-header"><td colspan="3">${esc(CATEGORY_LABELS[cat])}</td></tr>`
    );

    for (const item of catItems) {
      const noteHtml = item.note
        ? `<br/><span class="hw-note">${esc(item.note)}</span>`
        : '';
      rows.push(
        `<tr>` +
        `<td>${esc(item.name)}${noteHtml}</td>` +
...
<p style="font-size:8.5pt;color:#616161;margin-bottom:8px">
  ${hardware.totalItemTypes} item type${hardware.totalItemTypes === 1 ? '' : 's'} recommended
</p>
...
export function generatePdfHtml(opts: PdfExportOptions): string {
  const {
    ...
    allParts, optimizationResults, settings, hardware,
  } = opts;
  ...
  const hardwareHtml = hardware ? renderHardwareSection(hardware) : '';
...
${hardwareHtml ? `<div class="page-break"></div>\n${hardwareHtml}` : ''}
